...
def audit_evidence(failures):
    evidence_files = [
        "docs/evidencias/01-health.png",
        "docs/evidencias/02-metrics.png",
        "docs/evidencias/03-prometheus-targets.png",
        "docs/evidencias/04-grafana-dashboard.png",
        "docs/evidencias/05-loki-logs.png",
    ]
    for relative_path in evidence_files:
        path = ROOT / relative_path
        require(path.exists(), f"Falta evidencia: {relative_path}", failures)
        if path.exists():
            require(path.stat().st_size > 0, f"Evidencia vacia: {relative_path}", failures)
