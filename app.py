"""
Microservicio REST - Evaluacion Parcial 3.
...
# Error handlers
# La API responde en JSON tambien para errores HTTP.
...
def calcular():
    """
    Endpoint de calculadora basica.

    Query params:
        a   (float): Primer operando
        b   (float): Segundo operando
        op  (str):   Operacion: suma | resta | multiplicacion | division
    ...
    try:
        ...
        return jsonify({
            "error": "Los parametros 'a' y 'b' deben ser numeros validos"
        }), 400
    ...
    if op not in OPERACIONES_VALIDAS:
        ...
        return jsonify({
            "error": f"Operacion '{op}' no valida. Use: {', '.join(OPERACIONES_VALIDAS)}"
        }), 400
    ...
    if op == "suma":
        ...
        if b == 0:
            ...
            return jsonify({"error": "Division por cero no permitida"}), 400
        resultado = a / b
